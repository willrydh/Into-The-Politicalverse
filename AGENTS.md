...
- The header must scroll with the document initially and after a downward release. Float it only for an upward gesture, open menu or keyboard access; retain its original height to prevent page jumps. Do not restore the rejected permanently hidden sticky parent/visible child trick or add a status-area overlay. Keep off-screen controls inert, account for large scroll deltas, and read layout only at direction/size changes. See `docs/operations/header-motion.md`.

...
- Keep the non-painting sticky host separate from the explicitly visible, transform-only banner layer. Never restore the timed visibility switch on the moving surface: it was replaced after repeated iOS blinking reports. Keep off-screen controls inert, reveal before Tab focus moves, reject scroll jitter and out-of-bounds bounce, and avoid per-scroll layout measurements. Do not reintroduce a custom fixed tint or blur over the status area. See `docs/operations/header-motion.md`.

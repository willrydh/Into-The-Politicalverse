...
- The header must scroll with the document initially and after downward release. Reveal it only for an upward gesture, open menu or keyboard access; retain its original height to prevent page jumps. Use the temporary fixed positioning wrapper only while revealed. On release it must become `display: contents`, removing its renderer while its single semantic navigation surface scrolls in document coordinates. Changing fixed to absolute on the same rendered box does not invalidate WebKit’s cached status-area color. Do not restore root-scroll counter-translation, a permanently hidden sticky parent/visible child, or a separate status-area overlay. Keep off-screen controls inert, account for large scroll deltas, and read layout only at direction/size changes. See `docs/operations/header-motion.md`.

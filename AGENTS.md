...
- Keep the semantic banner inside the non-header sticky surface: WebKit deliberately retains the tint of hidden sticky header elements. The surface must become visibility:hidden after sliding away and reveal before Tab focus moves. Do not reintroduce a custom fixed tint or blur layer over the status area; the owner requested its removal after confirming improved native scrolling. See the PWA display evidence in `docs/brand/README.md`.

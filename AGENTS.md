...
- All major UI must work on phone, tablet, laptop and wide desktop. Use the shared `--page`, `--page-inset`, `--page-gutter` and `--panel-pad` spacing tokens for public page surfaces rather than new per-view margins.
...
- Post-election comparisons use previous counts from the same signed 2026 result archive, keyed by party and declared comparable area. Preserve missing party baselines as null, exact vote-derived shares and separately reported seats. Keep 2026 first with compact 2022 counts/share-point/seat changes below, and explanations below tables. See `docs/operations/post-election-2026.md`.

- The homepage leads with official counted 2026 votes, shares and reported mandates from the shared live feed. Keep its preliminary national picture until the separate final count covers all districts; mark the result established only with full coverage, all 349 reported seats and the official protocol. Never add the two counting stages or substitute model seats. On source failure retain verified counts with a delay warning. See `docs/operations/home-results.md`.
- Before establishment, the current national model is the election-night V2 projection. Forecasts, the election-night model and the simulator must share the same validated live-feed generation and pause on unavailable data. Never backfill current odds/seats with the frozen pre-election model. Keep that model as an explicit archive/reference for honest retrospective comparison. The default simulator must reproduce the exported V2 constituency inputs exactly, not redistribute its national shares using 2022 geography. User scenarios pin their starting generation until reset. See `docs/operations/current-projection.md`.
...
- The homepage mandate bar and legend show parties with reported seats, never the full registry of zero-seat parties. Before mandates are available, the eight parliamentary parties may retain unknown values. Full vote tables remain separately expandable.
- Candidate sharing metadata and PNGs use the same profile selection and verified history as the page, including explicit counted/final status. Preserve source/content revisions, dynamic election years, scope, null/zero distinctions and cache invalidation on new elections or corrected counts. Complete signed personal counts may enter history before establishment, with an explicit counted label on rankings, profiles and sharing. Incomplete counts and staged data stay excluded; never relabel counted data as final. See `docs/operations/candidate-sharing.md`.
- Candidate sharing cards show at most five distinct profile placements, with national cohorts first and compact labels such as “Svenska moderater”. Deduplicate repeated RD total-vote cohort ranks, but preserve the result area for changes/shares. Use two light-weight text columns, with exact/high placements first and broader top-100 mentions on the right. Keep profile label tiers, source scope and competition ties. Include full source and leaderboard-method versions in image cache identity; another candidate’s corrected result can change this person’s rank. Never mix source generations or infer a rank from only the person’s own votes.
- Profile standings must use the same full-cohort leaderboard function before top-100 clipping. Preserve the method/source versions, geography, party filter, year, competition ties and exact drilldown URLs. Total RD votes sum distinct constituencies per person and actual party; shares/changes remain constituency-scoped. Down-ballot support is an explicitly labelled subset (>=100 votes, all reported list positions >=4), never a causal performance score.
...
- Missing vote comparisons use a dash in cells, with the specific reason in its accessible label and a concise visible footnote below affected tables. On leaderboards, keep it after pagination alongside the existing footnotes. Never repeat explanatory sentences inside narrow numeric columns; preserve numeric deltas and null/zero distinctions.
...
- Paired leaderboard year columns show the selected/latest election first, then its four-year baseline, on desktop and mobile. Move the associated vote count and ballot positions together; keep the selected year emphasized. The default year and archive range follow the enabled verified history years.
- Adding data is not authorization to redesign the leaderboard. Preserve its original desktop columns and density. The owner explicitly requested a mobile redesign on 8 September: permanently visible filter controls, optional explanations, paired election-year votes/list positions, and the selected comparison prominently displayed. Keep mobile name, geography and party together; share-point rankings must emphasize share points, not percentage vote change. Ballot positions are compact secondary text under existing vote counts, always on one line with ellipsis when needed. Do not show counts such as "2 valsedlar" in the collapsed row; full list details expand on demand.
...
- Preserve the pre-election reference checksum and freeze boundary. Never create a reference retrospectively or grade a partial count as final. Enabling a new history year requires verified geography, source coverage and conservative identity linking. The 2026 importer now admits complete signed personal counts with their establishment status, preserving all source checks. See `docs/operations/election-transition-2026.md`.
...
- The 2026 result explorer uses official 2026 GIS boundaries and signed district distributions for RD/RF/KF votes and 2022 comparisons, with independently retained counting stages. Never represent reported local votes as 2026 personal votes. Keep same-archive district reconciliation, null collection turnout, explicit non-comparable districts and lazy geometry routes; see `docs/operations/maps-2026.md`. The separate historical local explorer uses 21 administrative counties, 290 municipalities and 2022 district boundaries; never substitute Riksdag constituency codes for county codes. Retain exact municipality-to-constituency links for candidate tables.
...
- SVT Valu is a separately reviewed national POLL snapshot. Preserve the source index identity, hashes, rounding, actual retrieval time and sample context; never import the widget’s simulated votes/seats or feed the survey into official results, the frozen forecast or nowcast without a separately reviewed method change. See `docs/data-sources/svt-valu-2026.md`.
- Valnatten uses a locally clipped broadcast ident: preserve the original crown, transform/opacity-only decorative motion, offscreen/reduced-motion pausing, real polling-station closing time and explicit waiting/receiving/delayed states. Animation is not a data signal, and 20:00 is not a promised first-result time. Never animate or invent numerical election results.
- The optional election-night nowcast is a separate experimental MODEL. Preserve minimum-support gates, independent district signatures, same-archive reconciliation, collection-vote treatment, source/baseline identities and the frozen pre-election reference. Synthetic reporting-order stress tests are not an actual valnatt replay; sensitivity ranges are not calibrated confidence intervals. Model failure must never block official results. The owner requested unvalidated bloc-majority probabilities: label their unestablished accuracy directly beside the numbers, define victory as 175 seats (not forming a government), preserve counted votes and disclose unresolved OTHER simulations without changing the denominator. V2 selects regularized geographic swing using municipality-held-out reported results only. Preserve effective-sample, extrapolation and geography gates; never feed unreported outcomes into fitting or selection. Reproduce both historical elections and all twelve orders per election, including regressions, before release. Use the archived live-data commits for prospective evaluation only after a complete later final count. See `docs/methodology/election-nowcast-v2.md`.
...
- The `live-data` branch publishes only `election-2026.json` and `area-results-2026.json`; never merge it into `main`. The second file contains independently verified RD/RF/KF areas and municipal summaries. Preserve the exact two-file publication allowlist. Preserve independent preliminary/final-count snapshots and the last verified data on source failure. Never publish rehearsal data as production results.
...
- The owner authorized migration of `politicalverse.se` from Loopia DNS to its own Cloudflare zone. Public static releases remain on this repository's GitHub Pages workflow, using the custom domain and root base path. Cloudflare routes `/admin*` and `/insights/*` to `politicalverse-insights`, with its own EU D1 database. The separate public `politicalverse-sharing` Worker owns `/people*`, `/en/people*` and `/share/candidate/*` for candidate metadata/images; it has no access to private analytics or secrets. Do not reuse another project's hosting target, secrets, database or domain. Keep DNSSEC enabled after migration.
- Preserve the independent domain-health schedule and verification after both public deployment paths. Fail on incorrect DNS/DNSSEC, parking content or lost redirect paths; never use forced addresses as proof of ordinary public access. DNS changes remain deliberate, with no automatic nameserver rewrite or DNSSEC disablement. See `docs/operations/domain-health.md`.

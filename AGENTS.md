...
- The header must follow native document scrolling in both directions. Begin reveal only for an upward gesture; preserve the same document coordinate during partial reversals and stop when scrolling stops. Do not add a separate timed slide or replay batched scroll deltas at release. Retain its original height to prevent page jumps. Use the temporary fixed positioning wrapper only once fully revealed, or for an open menu/keyboard access. During tracking it must be `display: contents`, removing its renderer while its single semantic navigation surface scrolls in document coordinates. Changing fixed to absolute on the same rendered box does not invalidate WebKit’s cached status-area color. Do not restore root-scroll counter-translation, a permanently hidden sticky parent/visible child, or a separate status-area overlay. Keep off-screen controls inert and cache layout measurements on size changes, never per scroll frame. See `docs/operations/header-motion.md`.

...
- Keep one native sticky surface throughout visible header movement. The bounded document anchor responds immediately to fractional movement and stays unchanged through partial reversals. CSS owns both edge stops. Do not restore a direction dead zone, fixed/absolute swaps during visible movement, per-frame transforms, timed slides, shadows or a status-area overlay. Remove the generic positioning renderer only once fully offscreen; never hide a parent while painting a visible child. Keep the spacer and equal negative following margin balanced. Preserve keyboard/menu access and safe-area spacing. Test slow repeated reversals as well as end positions; native iPhone Safari/PWA acceptance remains distinct from browser emulation. See `docs/operations/header-motion.md`.

...
- Keep the semantic banner inside the non-header sticky surface: WebKit deliberately retains the tint of hidden sticky header elements. The surface must become visibility:hidden after sliding away and reveal before Tab focus moves. Preserve the filtered standalone shade; a new unfiltered fixed surface can produce another solid status-area extension. See the PWA display evidence in `docs/brand/README.md`.

...
| --- | --- |
| [Signed result archives and technical specification](https://www.val.se/valresultat-och-statistik/statistik-och-data/teknisk-beskrivning-av-resultatfiler) | National Riksdag results and 29 constituencies. Both official rehearsal phases pass RSA/SHA-256 verification, vote reconciliation and independent fixed/adjustment/total seat checks. On 13 September the production index contains the official empty-file marker before publication; no production votes have been received yet. |
...
The national mandate JSON supplies national and constituency vote counts. The optional experimental valnattsprognos additionally verifies and reads the preliminary district vote JSON from the same archive, with its own signature, size limit and reconciliation gates. Model failure leaves official counts available. See `docs/methodology/election-nowcast-v1.md`; first production district-file compatibility is still awaiting publication. Downloads, archive paths, decompressed size, election identity, source clock, revision and totals are bounded/validated.

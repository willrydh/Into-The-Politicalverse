...
### Index/archive publication consistency

Live requests explicitly revalidate caches. If the ZIP differs from the index checksum, the collector rereads the same official phase index. It accepts the already-downloaded bytes only if their checksum matches that refreshed index, otherwise it retries the ZIP at most twice with a 15-second pause. Missing/unsafe phase entries and persistent mismatches fail closed. The signed JSON is verified independently afterward, and revision, arithmetic and cross-file gates still apply. No checksum or certificate check is bypassed. This handles either file being published first without turning a source-integrity failure into accepted data.

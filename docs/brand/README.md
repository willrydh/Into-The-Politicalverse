...
On 7 September the owner reported that the initial metadata/viewport change still left the white status area in both the installed PWA and iOS Safari. WebKit's [header-visibility change](https://github.com/WebKit/WebKit/commit/fc8c073ca9650f6b1e4c52e9f75667b6bb539228) deliberately retains a sampled tint for a hidden sticky `header`. In [Page::updateFixedContainerEdges](https://github.com/WebKit/WebKit/blob/main/Source/WebCore/page/Page.cpp), a non-header container with hidden visibility is released instead. This matches the site's previous transform-only hiding and is the basis of the follow-up. The owner subsequently confirmed improvement on the device, while reporting a lagging transparent layer during the transition.

The sticky `.site-header` is a generic surface containing the semantic `header` banner. It retains its layout height while sliding out, then becomes `visibility:hidden`; showing it clears that visibility immediately. `useScrollHeader` reveals it on upward scrolling, focus, and capture-phase Tab so keyboard controls stay reachable. Reduced motion clears the exit delay. The header has no outer shadow or shadow transition; its border stays inside the moving surface.

The custom standalone `body::before` overlay has been removed at the owner's request. It combined a fixed translucent gradient and a backdrop blur over the status area, even while the header moved away. The page now has no separate fixed tint/blur layer at that edge. Do not replace it with another overlay or change the working header transition to compensate; native smoothness and status-icon contrast still need visual verification on the device.

After that removal, the owner clarified that a weaker translucent strip was still visible only during movement. The header's remaining outer shadow and its 200 ms fade have now also been removed. The slide timing, hidden visibility and safe-area spacing are unchanged. This removes a verified source of paint outside the moving edge; it does not prove that the owner's native iOS artifact is fully resolved.

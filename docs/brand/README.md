...
The root-scroll translation introduced afterward also failed device acceptance: the owner’s 8 September screenshot showed page content behind the clock while the revealed navigation began below it, in both Safari and the installed Home Screen app. The current revision uses a temporary fixed positioning wrapper for upward reveal, allowing native status-area integration. During downward release the generic wrapper becomes `display: contents`; its renderer disappears, preventing WebKit from retaining it as the previous fixed container. The same opaque navigation surface, including safe-area padding, continues scrolling in document coordinates. The placeholder preserves document height without painting or sticking. There is no per-frame root-scroll translation, duplicated navigation or persistent status overlay. Keyboard access, mobile menu pinning and reduced motion remain available.

...
The replacement starts with ordinary document flow. The owner confirmed that the first downward scroll worked but a subsequent up/down cycle restored the defect. Upward reveal must therefore also avoid fixed/sticky positioning. A root-scroll translation holds the same opaque, absolute-positioned header in view, including its safe-area padding. Downward movement releases it into the document at its current position so the browser scrolls it away directly. Once outside the viewport, it returns to its original place, hidden and inert; no fixed/sticky header is created at any stage of the cycle. The placeholder only preserves document height and is never fixed, transformed, hidden or painted. Keyboard access, mobile menu pinning and reduced motion remain available.

...
Method: `candidate-leaderboards-1.1.0` (8 September 2026). Source normalization stays `candidate-history-1.0.2`.

Version 1.1.0 changes the editorial down-ballot cutoff from position 6 to position 4 at the owner’s request. Positions 1–3 are excluded on every reported ballot; positions 4 and 5 now qualify when the existing 100-vote requirement is met. Profile standings are regenerated with the same rule. Official observations, other leaderboard metrics and their ranking rules remain unchanged.
...
- **Support down the ballot**: an explicit exploratory subset, not an estimate of political competence or a causal adjustment. At least 100 personal votes; nonempty printed positions; every position reported in the result must be 4 or lower on the ballot (numerically >=4). Rank by personal votes / all valid votes for the actual party in that area. No historical match is required. A candidate reported at positions 1 and 20 does not qualify; absent positions do not qualify. `lists` counts source observations and cannot measure completeness against the deduplicated `ballotPositions` array. Other registered ballots may be absent from the source. The 100-vote and position-4 cutoffs are editorial choices made explicit beside the results. The ratio still varies with party size, geography, name recognition and campaign; it is not a residual performance score.

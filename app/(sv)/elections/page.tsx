...
import { partyDisplayName } from "@/lib/party-labels";
            ...
            <div className="archive-content">
              <div className="archive-outcomes">
                <section className="archive-blocs" data-classification="DERIVED" aria-label={sv ? "Blockresultat" : "Bloc result"}>
                  ...
                  <h3><PartyText>{leader?.name[locale] ?? (sv ? "Delat största block" : "Tied largest blocs")}</PartyText></h3>
                  ...
                  <dl className="archive-bloc-totals">{result.blocs.map(b => <div key={b.id}><dt><i className={`archive-tone-${b.tone}`} /><span><PartyText>{b.name[locale]}</PartyText>{b.name[locale] !== codes(b.parties, election.year) && <small><PartyGroup parties={b.parties} year={election.year}/></small>}</span></dt><dd>{f(b.seats)} <small>{sv ? "mandat" : "seats"}</small></dd></div>)}</dl>
                </section>
              ...
              <details className="archive-sources"><summary>{sv ? "Partiernas mandat, källor och blockindelning" : "Party seats, sources and bloc definitions"}</summary><p>{sv ? "Mandaten är slutliga valresultat. Blocken summerar de partier som anges för respektive valår. Regeringsrutan visar den första regeringen efter valet, inklusive en regering som fortsatte; senare skiften under mandatperioden ingår inte. Folkpartiet liberalerna bytte senare namn till Liberalerna. Loggorna visar partiernas nuvarande symboler." : "Seats are final election results. Each bloc sums the listed parties for that election year. The government panel shows the first government following the election, including a continuing government; later changes during the term are not included. The Liberal People’s Party was later renamed the Liberals. Logos show the parties’ current symbols."}</p><StaticSortableTable columns={[{key:"party",label:sv?"Parti":"Party",name:sv?"Parti":"Party",direction:"ascending"},{key:"seats",label:sv?"Mandat":"Seats",name:sv?"Mandat":"Seats"}]} rows={Object.entries(result.seats.parties).filter(([, seats])=>seats>0).map(([id,seats])=>({key:id,values:{party:translateText(partyDisplayName(id as PartyId,election.year),locale),seats},content:<tr key={id}><th scope="row"><span className="party-name-label"><PartyMark party={PARTIES[id as PartyId]} size="sm" label={partyDisplayName(id as PartyId,election.year)}/>{partyDisplayName(id as PartyId,election.year)}</span></th><td>{seats}</td></tr>}))} footer={<tfoot><tr><th>{sv?"Totalt":"Total"}</th><td>{ARCHIVE_TOTAL_SEATS}</td></tr></tfoot>}/><ul>{[result.seats.sourceId, ...government.sources].map(id => { const source = electionArchive.sources[id]; return <li key={id}><a href={source.url} target="_blank" rel="noreferrer">{source.publisher}: {source.title[locale]} ↗</a></li>; })}</ul></details>
            </div>
          </article>;
        })}

...
The separate `/admin/` insight dashboard uses a Cloudflare Worker and EU D1. Its prepared measurement contract requires consent, excludes authenticated administrators and keeps visits separate from identifiable people. The bilingual public consent UI and first-party collector connect visits, navigation, device/screen categories and technical quality to admin after explicit permission. Google account setup remains pending and no Google tag is included. The public site remains usable independently of measurement. See [insights operations](docs/operations/insights.md) for release status and activation checks.

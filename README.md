...
The header scrolls away with the document. A deliberate upward scroll brings the same navigation back; downward movement releases it to native page scrolling again. There is no permanent sticky host or hidden fixed layer at the screen edge. Active-page styling and the localized “You are here” trail remain, including verified map geography. Keyboard access, menu pinning, reduced motion and anchor offsets are preserved. See [header motion](docs/operations/header-motion.md).

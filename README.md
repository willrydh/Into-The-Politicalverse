...
The header scrolls away with the document. A deliberate upward scroll brings the same navigation back; downward movement releases it to native page scrolling again. The header never uses fixed or sticky positioning, including after it has been revealed. A root-scroll translation holds it in view until the next downward release. Active-page styling and the localized “You are here” trail remain, including verified map geography. Keyboard access, menu pinning, reduced motion and anchor offsets are preserved. See [header motion](docs/operations/header-motion.md).

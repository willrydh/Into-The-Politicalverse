...
The header uses one native sticky surface throughout visible motion. A bounded document anchor makes the first fractional scroll move it immediately, preserves partial reversals and pauses, and lets CSS stop it at both viewport edges. There is no eight-pixel dead zone, fixed/absolute swap at full reveal, timer or transform. The positioning renderer is removed only after the surface is entirely offscreen. A preceding spacer and equal negative following margin preserve the page layout. Menu/keyboard pinning, safe-area spacing and localized location remain. See [header motion](docs/operations/header-motion.md).

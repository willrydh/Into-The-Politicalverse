...
The site header hides when scrolling down and returns when scrolling up. Its banner moves on one persistent transform layer, without a timed visibility switch or shadow fade. The controller rejects scroll jitter and overscroll rebound. An active menu item and a localized “You are here” trail identify the current page; the map adds its verified county, municipality and district with links back through the hierarchy. Keyboard access, reduced motion and anchor offsets remain available. See [header motion](docs/operations/header-motion.md).

The iPhone web-app configuration requests a translucent status bar and a full-screen viewport. Top safe-area spacing belongs to the disappearing banner, inside a sticky host that never paints. Upward scrolling and keyboard navigation reveal the banner without changing its painted visibility. Menus and footer controls retain device-safe margins. No custom fixed tint or blur layer sits over the status area. Actual iPhone Safari/PWA rendering remains a separate acceptance check. See [PWA display behavior](docs/brand/README.md#pwa-display).

...
The separate `/admin/` insight dashboard uses a Cloudflare Worker and EU D1. Its prepared measurement contract requires consent, excludes authenticated administrators and keeps visits separate from identifiable people. The bilingual public notice has a single “Godkänn” / “Accept” button; its click activates the first-party collector for visits, navigation, device/screen categories and technical quality. The separate statistics-preferences controls and reopening function were removed from the footer and privacy page at the owner’s request on 11 September. Existing saved choices and the initial approval notice are preserved. Google account setup remains pending and no Google tag is included. The public site remains usable independently of measurement. See [insights operations](docs/operations/insights.md) for release status and activation checks.

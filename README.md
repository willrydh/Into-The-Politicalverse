...
The header follows native document scrolling in both directions. After a deliberate upward gesture, it enters at the same pace as the page and pins only when fully shown; stopping or reversing a partial reveal preserves its position. There is no separate slide animation or catch-up jump. The fully revealed navigation uses a temporary fixed positioning box so iOS can extend its opaque background into the status area. Downward movement removes that box with `display: contents`; merely changing its position would leave WebKit able to retain the old edge color. Active-page styling and the localized “You are here” trail remain, including verified map geography. Keyboard access, menu pinning and anchor offsets are preserved. See [header motion](docs/operations/header-motion.md).

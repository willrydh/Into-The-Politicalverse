...
The site header hides when scrolling down and returns when scrolling up, without an outer shadow or fading shadow trail. An active menu item and a localized “You are here” trail identify the current page; the map adds its verified county, municipality and district with links back through the hierarchy. Keyboard access, reduced motion and anchor offsets remain available.

The iPhone web-app configuration requests a translucent status bar and a full-screen viewport. Top safe-area spacing belongs to the disappearing header. Its sticky surface becomes hidden after sliding away, releasing the toolbar tint that iOS Safari can retain for an off-screen semantic header. The banner remains inside that surface; upward scrolling and keyboard navigation reveal it. Menus and footer controls retain device-safe margins. No custom fixed tint or blur layer sits over the status area. Actual iPhone Safari/PWA rendering remains a separate acceptance check. See [PWA display behavior](docs/brand/README.md#pwa-display).

...
The header now follows the owner-supplied CodePen reference: downward scrolling hides the whole top header and upward scrolling reveals it using the same 0.3-second CSS transition. One sticky box retains its layout space; direction changes only select the transform endpoint. The former document anchor, spacer, negative margin and display-mode switching are removed. Safe-area padding moves with the header; menu/keyboard access and reduced motion are preserved. See [header motion](docs/operations/header-motion.md).

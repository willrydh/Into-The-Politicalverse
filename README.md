...
The header scrolls away with the document. A deliberate upward scroll brings the same navigation back in a temporary fixed positioning box, so iOS can extend its opaque background into the status area. Downward movement removes that box with `display: contents` and releases the navigation to native page scrolling; merely changing its position would leave WebKit able to retain the old edge color. Active-page styling and the localized “You are here” trail remain, including verified map geography. Keyboard access, menu pinning, reduced motion and anchor offsets are preserved. See [header motion](docs/operations/header-motion.md).

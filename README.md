...
The header keeps the owner-supplied CodePen direction transition: down hides navigation and up reveals it with the same 0.3-second slide. The branding/utility row scrolls away naturally before the remaining navigation sticks. Only navigation and the location trail return while reading; the full header returns at page top. On mobile, a Menu button sits beside the location trail in this compact state. One sticky box retains its layout space, with no spacer or negative margins. Safe-area padding, keyboard/menu access and reduced motion remain. See [header motion](docs/operations/header-motion.md).

...
export type LocalSelection = { county: string; municipality: string; district: string; constituency: string; party: PartyId; year: LocalYear; metric: LocalMetric };
export function readLocalSelection(query: string, model: LocalElectionIndex): LocalSelection {
  ...
  return { county, municipality: municipality?.code ?? "", district: municipality && (district.startsWith(municipality.code) && /^\d{8}$/.test(district) || district === `${municipality.code}-collection`) ? district : "", constituency: model.national.constituencies?.includes(constituency) ? constituency : "", party: PARTY_IDS.includes(party) ? party : "S", metric: metric === "swing" || metric === "turnout" ? metric : "share", year: metric === "swing" ? 2022 : LOCAL_YEARS.includes(year) ? year : 2022 };
}

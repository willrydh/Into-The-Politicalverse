...
import { validateSharePerson, type SharePerson, type ShareScope } from "./sharing";
...
export function buildSharePerson(person: Person): SharePerson {
  ...
  const payload = { id: person.id, name: person.name, defaultElection: defaults.election, scopes };

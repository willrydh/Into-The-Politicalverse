...
function ranking(r: RankingRow) {
  ...
  if (c.previous) assert(c.previous.year === r.year - 4 && [c.previous.votes, c.previous.partyVotes].every(n => Number.isSafeInteger(n) && n >= 0) && c.previous.votes <= c.previous.partyVotes && typeof c.previous.partyCode === "string" && typeof c.previous.partyName === "string" && PARTY_IDS.includes(c.previous.partyId) && c.previous.partyId === candidatePartyId(c.previous.partyCode));

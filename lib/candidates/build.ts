...
export function buildCandidateData() {
  ...
  const catalog: CandidateCatalog = { schemaVersion: 1, version, methodVersion: CANDIDATE_METHOD, publisher: "Valmyndigheten", retrievedAt: manifest.retrievedAt, years: [...CANDIDATE_YEARS], people: people.length, linkedPeople: people.filter(p => p.linked).length, electionIdentities: identityInputs.length, results: people.reduce((sum,p)=>sum+p.results.length,0), counties: geography.counties.map(c=>({code:c.code,name:c.name})), municipalities: geography.municipalities.map(m=>({code:m.code,name:m.name,parent:m.parent!})), constituencies: rdAreas.map(c=>({...c,county:geography.municipalities.find(m=>m.constituencies?.includes(c.code))!.parent!})), sources: manifest.sources };

...
The header uses normal document flow initially. Upward gestures reveal it inside a temporary fixed `.site-header-positioner`; this lets iOS recognize the visible navigation when extending its status-area background. On downward release that generic wrapper becomes `display: contents`, removing its renderer immediately while the same `.site-header` surface scrolls from its absolute document coordinate. Do not replace this with fixed-to-absolute on the same rendered box: WebKit retains the last fixed container’s edge color while its renderer survives. The empty height placeholder prevents content jumps. The rejected hidden-parent/visible-child trick, custom tint overlays and root-scroll translation have been removed. Localized location, keyboard access, menu pinning, safe-area padding and reduced motion remain. See `docs/operations/header-motion.md`.

...
`siteViewport` enables `viewport-fit=cover` in both language roots and the exported 404. The latter keeps a small server wrapper for viewport metadata and the existing client language resolver in `app/global-not-found-document.tsx`. Shared Apple metadata uses `black-translucent`; the safe-area top padding is part of the translated header, never fixed spacing on the body. Standalone status shading is translucent, pointer-transparent and occupies no layout space. Safe-area emulation verifies layout but does not prove the installed iPhone status bar; see the PWA acceptance steps in `docs/brand/README.md`.

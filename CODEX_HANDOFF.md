...
Analytics follow-up, 9 September 2026: authenticated production APIs and D1 confirmed zero recorded visits/events: the public collector was never shipped. Worker version `44a9c9b5-dc63-4bc0-bfff-83c0ba51ea50` now reports unverified empty collection/last received signal, clears stale realtime rows on errors, and prevents late performance samples moving or reactivating a departed visit. Its authenticated production checks passed, with all five public resource checks HTTP 200. Source PR #54 is merged. The remaining local frontend draft is on `codex/visitor-collector-ready`. No visitor events were inserted during production verification.

The owner approved the public privacy contact on 9 September. This release wires the consented first-party collector into both languages and the global 404, adds privacy/settings routes, and includes ten isolated browser-lifecycle tests. It records visits, route transitions, coarse devices/screens, feature use and field performance after explicit permission. Withdrawal stops pending collection, including across tabs; blocked browser storage has a document-only fallback. The integrated checks and mobile Swedish/English preference checks passed. Verify the deployed frontend with a real consented browser visit through realtime/report/D1, withdrawal and admin exclusion. Historical uncaptured traffic is unavailable. Google Analytics remains unconnected and no Google tag is included; a future connection needs verified stream settings, updated disclosure and renewed consent for that recipient.

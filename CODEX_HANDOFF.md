...
The owner rejected the extra visual weight introduced with ballot positions. Preserve the original leaderboard columns and mobile proportions; the positions are compact secondary disclosures under existing vote counts. Adding information must not become an unsolicited redesign.
...
The header follows the owner-supplied CodePen direction-driven slide, adapted to the top edge: one sticky box, a shared 0.3-second transform transition, down hides and up reveals. The document-anchor/spacer/display-switching implementation was removed. Safe-area padding moves with the surface; menu/keyboard access and reduced motion remain. See `docs/operations/header-motion.md`; actual iPhone acceptance is separate from browser QA.

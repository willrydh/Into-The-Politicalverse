...
2026-09-24: Complete signed personal-vote counts now enter rankings, profiles, comparisons, search and sharing before the separate mandate/protocol establishment step. Status stays explicit: counted is not final. This fixes Mark and other fully counted areas previously hidden by the establishment gate. Partial counts remain excluded, and national/county profile standings still require their full final cohort. See [candidate/results operations](docs/operations/candidate-results-2026.md).
...
The homepage now leads with official counted 2026 votes, vote shares and reported mandates from the shared verified live feed. It retains the preliminary national picture while the separate final count starts, then switches when that count covers all districts. A final label additionally requires the published protocol and all 349 reported seats. See `docs/operations/home-results.md`. Until establishment, the forecast terminal, election-night model and default simulator use the same `nowcast.estimate` generation. Established results now supersede that current model. Seat totals and coalition sums use the published V2 rows; majority probabilities use its exact simulations and denominator. No pre-election probabilities fill gaps. The forecast terminal retains the frozen pre-election UI in an explicitly collapsed archive; its JSON endpoint declares the archive role and live replacement. The collector additionally exports its exact rounded constituency seat inputs, checked against the central projection before the simulator accepts them. A user-edited scenario pins that source generation until reset; the untouched simulator follows live updates. See `docs/operations/current-projection.md`.
...
The election-night model includes owner-requested, explicitly unvalidated bloc-majority simulations. V2 includes historical joint errors, geographic residuals, model disagreement and turnout stress. Victory means 175 seats, not government formation. See [method and assumptions](docs/methodology/election-nowcast-v2.md).

Election-night projection v2, 13 September: regularized district regression, partially pooled geography and municipality-held-out model selection replace a uniform national swing when supported by reported data. Support gates can release predictions before 5% coverage, requiring effective sample size and bounded extrapolation. Evaluation compares both estimators on 4,631 physical district comparisons for 2018 and 4,162 for 2022, with twelve synthetic orders per election. It retains regressions and separate final-count errors. These are development tests, not actual reporting-time replays or probability calibration. Official counts and the frozen pre-election reference remain independent. See [method, evaluation and prospective audit](docs/methodology/election-nowcast-v2.md).
...
The homepage retains its crown, compact freshness strip and inward-facing mandate board. Its hero presents the official count status, district coverage, valid votes and turnout in reported districts. A sortable vote/share/seat table follows, with accessible party logos on narrow screens. The model and frozen forecast comparison remain available through separate links.
...
Analytics follow-up, 9 September 2026: authenticated production APIs and D1 confirmed zero recorded visits/events: the public collector was never shipped. Worker version `44a9c9b5-dc63-4bc0-bfff-83c0ba51ea50` now reports unverified empty collection/last received signal, clears stale realtime rows on errors, and prevents late performance samples moving or reactivating a departed visit. Its authenticated production checks passed, with all five public resource checks HTTP 200. Source PR #54 is merged. Frontend PR #55 was subsequently merged and deployed at `27baf3c`. A real consented browser visit verified two pageviews, the start-to-rankings transition, dimensions, filter events, withdrawal and admin exclusion; its exact test visit was removed afterward.

The owner approved the public privacy contact on 9 September. This release wires the consented first-party collector into both languages and the global 404, adds privacy/settings routes, and includes ten isolated browser-lifecycle tests. It records visits, route transitions, coarse devices/screens, feature use and field performance after explicit permission. Withdrawal stops pending collection, including across tabs; blocked browser storage has a document-only fallback. The integrated checks and mobile Swedish/English preference checks passed. The public frontend was verified through realtime/report/D1 after deployment. Historical uncaptured traffic is unavailable. Google Analytics remains unconnected and no Google tag is included; a future connection needs verified stream settings, updated disclosure and renewed consent for that recipient.

The owner requested a single initial-notice button, “Godkänn” / “Accept”, on 9 September. Its click is the active approval; browsing alone never starts collection. The short notice explains that Politicalverse is free and statistics helps continued development. On 11 September, the owner requested removal of “Statistikval” and its function. The shared settings control, reopening event, preferences state and both footer/privacy entry points are removed. The initial approval notice and existing saved choices remain unchanged.
...
After the 9 September DNS incident, `domain-health.yml` independently checks public DNSSEC, the expected nameserver pair, system DNS, ordinary HTTPS identity and exact redirects on a fifteen-minute schedule and after both Pages deployment paths. Preserve its read-only scope and bounded retries. Do not equate a direct origin check or HTTP 200 parking response with working public access. See `docs/operations/domain-health.md` and `docs/operations/dns-migration-2026-09-09.md`.

Candidate profile links now have a separate public sharing Worker for crawler metadata and 1200 × 630 PNGs. Every Pages candidate prebuild regenerates compact sharing shards and per-profile content revisions; accepted future election years and count corrections refresh images without hardcoded year changes in the Worker. Existing profile selection/layout is preserved. Complete verified 2026 counts carry counted/final status; staged data and incomplete counts remain excluded. Cards also include up to five distinct profile leaderboard placements for the selected standings year, prioritizing national results and then the strongest relevant placements. Two columns use regular-weight metric labels, with exact/high ranks leading and broader top-100 mentions on the right. Compact labels identify each election, geography and party cohort. Standings refresh when the full source cohort changes. See `docs/operations/candidate-sharing.md` for cache semantics, tests and explicit Worker deployment.
...
The owner rejected the extra visual weight introduced with ballot positions. Preserve the original desktop leaderboard columns; the positions are compact secondary disclosures under existing vote counts, always one line with ellipsis if necessary. Do not display extra ballot-count text such as "2 valsedlar" in collapsed rows. Adding information must not become an unsolicited redesign.

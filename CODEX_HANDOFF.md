...
The shared header now separates a non-painting sticky host from an explicitly visible, persistent transform layer containing the semantic banner. The old timed visibility switch was replaced after the owner still reported iOS blinking following both shadow/overlay removals. The scroll controller uses cached dimensions, peak/trough hysteresis and out-of-bounds rejection. Off-screen controls are inert; Tab reveals them immediately without scrolling a translated link into view. Native acceptance is still pending. The localized location trail continues to identify each page and verified geographic scope. Preserve active-page semantics, mobile menu pinning, reduced motion and anchor offsets; see `docs/operations/header-motion.md`.

`siteViewport` enables `viewport-fit=cover` in both language roots and the exported 404. The latter keeps a small server wrapper for viewport metadata and the existing client language resolver in `app/global-not-found-document.tsx`. Shared Apple metadata uses `black-translucent`; the safe-area top padding belongs to the disappearing banner. No body top inset or fixed tint/blur overlay is present. Keep the host's non-painting state and the child's painted visibility stable during scrolling. Browser checks do not prove native iPhone rendering; see the evidence and acceptance steps in `docs/brand/README.md`.

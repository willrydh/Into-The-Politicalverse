...
The owner rejected the extra visual weight introduced with ballot positions. Preserve the original leaderboard columns and mobile proportions; the positions are compact secondary disclosures under existing vote counts, always one line with ellipsis if necessary. Do not display extra ballot-count text such as "2 valsedlar" in collapsed rows. Adding information must not become an unsolicited redesign.
...
The owner-supplied CodePen slide now returns only navigation and the location trail after scrolling. Native sticky positioning lets the branding/utility row scroll away first; the full header returns at page top. Mobile exposes a Menu button beside the location trail while compact. Keep one sticky box, the same 0.3-second transform transition in both directions, safe-area padding and menu/keyboard access. No document-anchor/spacer balancing. See `docs/operations/header-motion.md`; actual iPhone acceptance is separate from browser QA.

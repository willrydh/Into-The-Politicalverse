...
`siteViewport` enables `viewport-fit=cover` in both language roots and the exported 404. The latter keeps a small server wrapper for viewport metadata and the existing client language resolver in `app/global-not-found-document.tsx`. Shared Apple metadata uses `black-translucent`; the safe-area top padding belongs to the disappearing header. The owner reported that the initial metadata-only fix still left a white status area in both the installed PWA and iOS Safari. The follow-up keeps the semantic banner inside a non-header sticky surface, sets visibility:hidden after its exit and reveals it before Tab focus moves. This avoids WebKit's explicit retention of hidden header tints. Standalone shading uses a translucent filtered backdrop to avoid another solid color extension. Browser checks do not prove native iPhone rendering; see the evidence and acceptance steps in `docs/brand/README.md`.

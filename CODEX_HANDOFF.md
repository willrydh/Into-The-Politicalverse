...
The header uses normal document flow initially and document-based positioning throughout every later cycle. Upward gestures reveal the absolute-positioned header using a root-scroll translation; downward movement releases that translation at its current document position. Never reintroduce fixed/sticky positioning on upward reveal: the owner confirmed it brought the iOS strip back on the next downward scroll. Native scrolling moves the released header, with no timed exit animation. The empty height placeholder stays at the document start and prevents content jumps. The rejected permanently hidden sticky parent and visible child layer have been removed. The localized location trail, keyboard access, mobile menu pinning, safe-area padding and reduced motion remain. See `docs/operations/header-motion.md`.

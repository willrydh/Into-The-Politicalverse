...
Most personal votes is the default leaderboard. The Riksdag total-vote view groups distinct constituency observations by candidate and actual party within the selection, with expandable source counts; comparisons remain constituency-scoped. The explicit “Stöd längre ned” view ranks candidates with >=100 personal votes and every reported printed list position >=6 by their share of the party’s votes. It does not estimate a causal list-position effect. Candidate profiles expose linked top-100 standings by election year, scope and all-parties/own-party cohort, generated with the same ranking function. Compact large percentage labels retain exact accessible values. See `docs/methodology/candidate-history-v1.md` for method `candidate-leaderboards-1.0.0` and limitations.

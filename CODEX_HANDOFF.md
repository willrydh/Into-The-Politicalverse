...
The shared header is sticky with direction-based visibility: it hides on downward scrolling, reappears on upward scrolling and remains accessible to keyboard focus. Its outer shadow and shadow fade have been removed after the owner reported a weaker translucent strip during movement even after removal of the status overlay; native acceptance is still pending. Its localized location trail identifies every site page and receives verified geographic names from the local explorer through `SiteLocationProvider`; stale route/query entries are ignored. Preserve active-page semantics, mobile menu centering, reduced-motion behavior and the measured header offset for in-page navigation.

`siteViewport` enables `viewport-fit=cover` in both language roots and the exported 404. The latter keeps a small server wrapper for viewport metadata and the existing client language resolver in `app/global-not-found-document.tsx`. Shared Apple metadata uses `black-translucent`; the safe-area top padding belongs to the disappearing header. The semantic banner sits inside a non-header sticky surface, which becomes visibility:hidden after its exit and reveals before Tab focus moves. The owner confirmed improved native scrolling after this change, then reported a lagging transparent layer. The custom standalone `body::before` tint/blur has been removed; preserve the confirmed header behavior without reintroducing a fixed status overlay. Browser checks do not prove native iPhone rendering; see the evidence and acceptance steps in `docs/brand/README.md`.

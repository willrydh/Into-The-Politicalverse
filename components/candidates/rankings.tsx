...
import { CandidateLoading, CandidateMethod, electionLabel, partyLabel, partyAbbreviation, useCandidateResource, VoteDelta } from "./shared";
  ...
  return <div className="candidate-page">
    ...
    <div className="candidate-body">
      ...
      {!catalog?<CandidateLoading error={catalogState.error} retry={catalogState.retry}/>:!data?<CandidateLoading error={state.error} retry={state.retry}/>:<>
        <p className="candidate-result-count" role="status">{f(ranking.length)} {sv?"resultat i topplistan":"leaderboard entries"}{search&&` · ${sv?"Placeringen behålls från hela urvalet":"Ranks are retained from the full selection"}`}</p>
        <div className="local-table-scroll candidate-scoreboard candidate-scoreboard--ranking"><table><thead><tr><th scope="col">#</th><th scope="col">{sv?"Kandidat / område":"Candidate / area"}</th><th scope="col">{sv?"Parti":"Party"}</th><th scope="col">{year-4}</th><th scope="col">{year}</th><th scope="col">{sv?"Förändring":"Change"}</th>{metric==="sharePoints"&&<th scope="col">{sv?"Andelslyft":"Share gain"}</th>}</tr></thead><tbody>{ranking.slice(currentPage*50,(currentPage+1)*50).map(({row:r,rank})=><tr key={`${r.person}:${r.areaCode}:${r.partyCode}`}><td className="candidate-rank">{rank}</td><th scope="row"><Link href={localizedHref(personHref(r.person,election,r.areaCode),locale)}>{r.name}<span aria-hidden="true"> ↗</span></Link><small>{r.areaName}</small></th><td><span className="candidate-party-tag">{partyAbbreviation(r)}</span>{r.comparison.previous&&r.comparison.previous.partyCode!==r.partyCode&&<small className="candidate-party-change">{partyAbbreviation(r.comparison.previous)} → {partyAbbreviation(r)}</small>}</td><td data-label={String(year-4)}>{r.comparison.previous?f(r.comparison.previous.votes):"—"}</td><td data-label={String(year)} className="candidate-total">{f(r.votes)}</td><td><VoteDelta comparison={r.comparison}/></td>{metric==="sharePoints"&&<td data-label={sv?"Andelslyft":"Share gain"}>{r.comparison.sharePoints===null?"—":`+${f(r.comparison.sharePoints,2)} pp`}</td>}</tr>)}</tbody></table></div>

...
import { localSwing, localTurnout, voteShare } from "@/lib/data/geography/local-math";
...
function metricValue(area: LocalArea, s: LocalSelection) {
  ...
  return s.metric === "swing" ? localSwing(area, s.party) : s.metric === "turnout" ? localTurnout(result) : voteShare(result, s.party);
}
...
function History({ area, party, t, f, color }: { area: LocalArea; party: PartyId; t: LocalCopy; f: NumberFormat; color: string }) {
  ...
  return <section className="local-history">
    ...
    <div className="local-table-scroll"><table className="local-table"><thead><tr><th>{t.year}</th><th>{t.votes}</th><th>{t.share}</th><th>{t.turnout}</th></tr></thead><tbody>{values.map(v => <tr key={v.year}><th>{v.year}</th><td>{f(v.result.votes[party])}</td><td>{f(v.share, 2)}%</td><td>{localTurnout(v.result) === null ? "—" : `${f(localTurnout(v.result), 2)}%`}</td></tr>)}</tbody></table></div>
...
function AreaProfile({ area, parent, selection, t, f, onParty }: { area: LocalArea; parent: LocalArea | undefined; selection: LocalSelection; t: LocalCopy; f: NumberFormat; onParty: (party: PartyId) => void }) {
  ...
  const delta = localSwing(area, selection.party);
  return <article className="local-profile" aria-label={nameFor(area, t)}>
    ...
    {result ? <>
      ...
      <dl className="local-metrics"><div><dt>{t.swing}</dt><dd>{delta === null ? "—" : `${delta > 0 ? "+" : ""}${f(delta, 2)} pp`}</dd></div><div><dt>{t.turnout}</dt><dd>{localTurnout(result) === null ? "—" : `${f(localTurnout(result), 2)}%`}</dd></div><div><dt>{t.eligible}</dt><dd>{result.eligibleVoters ? f(result.eligibleVoters) : t.noTurnout}</dd></div><div><dt>{t.totalVotes}</dt><dd>{f(result.totalVotes)}</dd></div></dl>
    ...
    {result && <details className="local-details"><summary>{t.allParties}</summary><table className="local-table"><thead><tr><th>{t.party}</th><th>{t.votes}</th><th>{t.share}</th></tr></thead><tbody>{[...PARTY_ORDER].sort((a, b) => result.votes[b] - result.votes[a]).map(p => <tr key={p}><th><button type="button" onClick={() => onParty(p)}>{p === "OTHER" ? t.other : PARTIES[p].shortName}</button></th><td>{f(result.votes[p])}</td><td>{f(voteShare(result, p), 2)}%</td></tr>)}</tbody></table></details>}
  </article>;
}
...
function AreaList({ areas, selection, selected, onSelect, t, f }: { areas: LocalArea[]; selection: LocalSelection; selected: string; onSelect: (area: LocalArea) => void; t: LocalCopy; f: NumberFormat }) {
  ...
  return <section className="local-area-list"><div className="local-section-title"><h2>{t.list}</h2><span>{f(filtered.length)} {t.count}</span></div>
    ...
    <div className="local-table-scroll"><table className="local-table local-area-table"><thead><tr><th>{t.area}</th><th>{selection.metric === "swing" ? t.swing : selection.metric === "turnout" ? t.turnout : t.share}</th><th>{t.votes}</th><th>{t.turnout}</th></tr></thead><tbody>{filtered.map(area => {
      const v = metricValue(area, selection); const r = area.results.find(r => r.year === selection.year);
      return <tr key={area.code} className={selected === area.code ? "is-selected" : ""}><th><button type="button" onClick={() => onSelect(area)} aria-label={`${t.open} ${nameFor(area, t)}`}>{nameFor(area, t)}<span aria-hidden="true">→</span><small>{area.code}</small></button></th><td>{v === null ? "—" : `${selection.metric === "swing" && v > 0 ? "+" : ""}${f(v, 2)}${selection.metric === "swing" ? " pp" : "%"}`}</td><td>{r ? f(r.votes[selection.party]) : "—"}</td><td>{localTurnout(r) === null ? "—" : `${f(localTurnout(r), 2)}%`}</td></tr>;
...
export function LocalElectionExplorer({ model }: { model: LocalIndexModel }) {
  ...
  return <div className="local-explorer" style={{ "--local-party": PARTIES[selection.party].color } as CSSProperties}>
    ...
    <div className="local-filters">
      ...
      <label>{t.year}<select value={activeYear} disabled={selection.metric === "swing"} onChange={e => update({ year: Number(e.target.value) as LocalYear })}>{LOCAL_YEARS.map(year => <option key={year} value={year}>{year}</option>)}</select></label>
      <label>{t.metric}<select value={selection.metric} onChange={e => update({ metric: e.target.value as LocalSelection["metric"], ...(e.target.value === "swing" ? { year: 2022 } : {}) })}><option value="share">{t.share}</option><option value="swing">{t.swing}</option><option value="turnout">{t.turnout}</option></select></label>
    </div>
    ...
    <div className="local-workspace"><div className="local-map-column"><div className="local-section-title"><h2>{municipality ? t.districts : county ? t.municipalities : t.counties}</h2><span>{selection.metric === "swing" ? "2018 → 2022" : activeYear}</span></div>
    ...
    <LocalPersonalVotes area={selectedArea} selection={selection} constituencies={model.constituencies} onConstituency={constituency => update({ constituency })} />

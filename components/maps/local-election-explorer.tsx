...
export function LocalElectionExplorer({ model }: { model: LocalIndexModel }) {
  ...
  return <div className="local-explorer" id="local-results" style={{ "--local-party": PARTIES[selection.party].color } as CSSProperties}>
    ...
    <div className="local-filters">
      ...
      <label>{t.year}<select value={activeYear} onChange={e => { if(e.target.value === "2026") { const next = new URLSearchParams(query); next.set("year","2026"); next.delete("district"); navigateLocalQuery(`?${next}`); } else update({ year: Number(e.target.value) as LocalYear }); }}><option value="2026">2026</option>{[...LOCAL_YEARS].reverse().map(year => <option key={year} value={year}>{year}</option>)}</select></label>

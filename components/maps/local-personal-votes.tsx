...
export function LocalPersonalVotes({ area, selection, constituencies, onConstituency }: { area: LocalArea; selection: LocalSelection; constituencies: Array<{ code: string; name: string }>; onConstituency: (code: string) => void }) {
  const locale = useLocale(); const sv = locale === "sv";
  const options = constituencies.filter(c => area.constituencies?.includes(c.code));
  const code = options.some(c => c.code === selection.constituency) ? selection.constituency : options[0]?.code ?? "";
  ...
  return <section className="local-personal"><div className="local-section-title"><h2>{sv ? "Personröster" : "Personal votes"}</h2><span>2022 · {sv ? "Riksdagsvalkrets" : "Riksdag constituency"}</span></div>
    ...
    {selection.year !== 2022 ? <p className="local-notice">{sv ? "Personröster är inlästa för 2022. Välj 2022 ovan för att visa dem." : "Personal votes are available for 2022. Select 2022 above to view them."}</p> : <>
      <div className="local-list-controls"><label>{sv ? "Riksdagsvalkrets för personröster" : "Riksdag constituency for personal votes"}<select value={code} onChange={e => { onConstituency(e.target.value); setAll(false); }} disabled={options.length === 1}>{options.map(c => <option key={c.code} value={c.code}>{c.name}</option>)}</select></label><label>{sv ? "Sök kandidat" : "Search candidates"}<input type="search" value={search} onChange={e => setSearch(e.target.value)} /></label></div>

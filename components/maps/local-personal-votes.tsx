...
import { personHref, type CandidateYear } from "@/lib/candidates/types";
...
export function LocalPersonalVotes({ area, county, municipality, selection, constituencies, onConstituency }: { area: LocalArea; county?: LocalArea; municipality?: LocalArea; selection: Omit<LocalSelection, "year"> & {year: CandidateYear}; constituencies: Array<{ code: string; name: string }>; onConstituency: (code: string) => void }) {
  ...
  return <section className="local-personal" id="personal-votes" aria-labelledby="personal-votes-title">
    ...
    {municipality&&home&&<p className="local-note local-personal-scope">{sv?<>{municipality.name} hör till <strong>{home.name}</strong> i indelningen {selection.year}.</>:<>{municipality.name} belongs to <strong>{home.name}</strong> in the {selection.year} boundaries.</>} {options.length>1&&(sv?"Du kan välja en annan valkrets i länet. Valet gäller personrösterna; kartan behåller sitt område.":"You can select another constituency in the county. This changes the personal votes; the map keeps its selected area.")}</p>}
    ...
    <div className="local-share"><Link className="button" href={localizedHref(`/rankings/?election=RD&year=${selection.year}&area=${code}&party=${selection.party==="OTHER"?"":({M:"0001",S:"0002",L:"0003",C:"0004",V:"0005",MP:"0055",KD:"0068",SD:"0110"} as Record<string,string>)[selection.party]??""}`,locale)}>{sv?"Topplista i valkretsen":"Constituency leaderboard"} →</Link>{municipality&&<Link className="button" href={localizedHref(`/rankings/?election=KF&year=${selection.year}&area=${municipality.code}`,locale)}>{sv?`Kommunvalets personröster i ${municipality.name}`:`Municipal personal votes in ${municipality.name}`} →</Link>}</div>
    <a className="local-source-link" href={selection.year === 2026 ? "https://www.val.se/valresultat-och-statistik/statistik-och-data/radata-val-2026" : "https://www.val.se/valresultat-och-statistik/statistik-och-data/radata-fran-val-2002-2022"} target="_blank" rel="noreferrer">{sv?"Valmyndigheten: källor och arkiv":"Valmyndigheten: sources and archives"} ↗</a>
  </section>;
}

...
import { CandidateLoading, partyLabel, useCandidateResource, VoteDelta } from "@/components/candidates/shared";
...
export function LocalPersonalVotes({ area, county, municipality, selection, constituencies, onConstituency }: { area: LocalArea; county?: LocalArea; municipality?: LocalArea; selection: LocalSelection; constituencies: Array<{ code: string; name: string }>; onConstituency: (code: string) => void }) {
  ...
  return <section className="local-personal" id="personal-votes" aria-labelledby="personal-votes-title">
    ...
    <p className="local-note"><strong>{data?.name??options.find(c=>c.code===code)?.name} · {selection.party==="OTHER"?(sv?"Övriga partier":"Other parties"):partyLabel({partyId:selection.party,partyName:PARTIES[selection.party].name,year:selection.year},sv)}</strong></p>
    {!data?<CandidateLoading error={state.error} retry={state.retry}/>:<>
      <div className="local-table-scroll"><table className="local-table"><thead><tr><th>{sv?"Kandidat":"Candidate"}</th><th>{sv?"Personröster":"Personal votes"}</th><th>{selection.year-4} → {selection.year}</th><th>{sv?"Andel av partiets röster":"Share of party votes"}</th><th>{threshold} % {sv?"spärr":"threshold"}</th></tr></thead><tbody>{(all||search?candidates:candidates.slice(0,10)).map(c=><tr key={`${c.partyCode}:${c.id}`}><th><Link className="candidate-profile-link" href={localizedHref(personHref(c.person,"RD",code),locale)}>{c.name} ↗</Link>{selection.party==="OTHER"&&<small className="local-candidate-party">{partyLabel(c,sv)}</small>}{c.comparison.previous&&c.comparison.previous.partyCode!==c.partyCode&&<small className="local-candidate-party">{c.comparison.previous.partyId==="OTHER"?c.comparison.previous.partyName:c.comparison.previous.partyId} → {c.partyId==="OTHER"?c.partyName:c.partyId}</small>}</th><td>{f(c.votes)}</td><td><VoteDelta comparison={c.comparison}/></td><td>{personalVoteShare(c)===null?"—":`${f(personalVoteShare(c)!,2)} %`}</td><td>{c.partyVotes>0&&c.votes*100>=threshold*c.partyVotes?(sv?"Uppnådd":"Reached"):"—"}</td></tr>)}</tbody></table></div>

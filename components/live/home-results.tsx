...
export function HomeResults() {
  ...
  const mandateParties = parties.filter(p => (p.seats ?? 0) > 0 || (!seatsComplete && p.seats === null && PARTY_CODE_TO_ID[p.code]));
  ...
  return <>
    <section className="forecast-hero result-home" data-classification="OFFICIAL" data-result-stage={result?.stage} data-result-revision={result?.sourceRevision}>
      ...
      <div className="mandate-board" aria-label={sv ? "Valmyndighetens mandatfördelning" : "Official seat allocation"}>
        ...
        <div className="mandate-bar">{seatsComplete && mandateParties.map(p => <span key={p.code} style={{ width: `${p.seats! / 349 * 100}%`, background: PARTIES[PARTY_CODE_TO_ID[p.code]]?.color ?? "var(--muted)" }} title={`${translateText(PARTIES[PARTY_CODE_TO_ID[p.code]]?.name ?? p.name, locale)}: ${p.seats}`} />)}</div>
        <div className="mandate-legend">{mandateParties.map(p => <div key={p.code}>{PARTIES[PARTY_CODE_TO_ID[p.code]] ? <PartyMark party={PARTIES[PARTY_CODE_TO_ID[p.code]]} size="sm" /> : <span>{p.abbreviation || p.name}</span>}<strong>{p.seats ?? "—"}</strong></div>)}</div>
    ...
    <section className="product-section result-home__results" id="roster-2026" data-classification="OFFICIAL" data-result-stage={result?.stage} data-result-revision={result?.sourceRevision}>
      ...
      <details className="local-details"><summary>{sv ? "När är rösterna sluträknade?" : "When is the count final?"}</summary>
        <p>{established ? (sv ? "Riksdagsvalet är fastställt. Valmyndighetens fullständiga räkning och protokoll ligger till grund för siffrorna ovan. Kommun- och regionvalen fastställs separat." : "The Riksdag result is final. The figures above use the authority’s complete count and protocol. Municipal and regional results are established separately.") : sv ? "Den slutliga rösträkningen börjar den 14 september. Sena förtids- och utlandsröster räknas vid uppsamlingsräkningen från den 16 september. Riksdagsresultatet väntas fastställas ungefär en vecka efter valdagen." : "The final count starts on 14 September. Late advance and overseas votes are included in the collection count from 16 September. The Riksdag result is expected to be established about a week after election day."}</p>
      ...
      <div className="result-home__links"><Link href={href("/maps/?year=2026")}>{sv ? "Kommun- och regionvalet 2026 →" : "Municipal and regional elections 2026 →"}</Link><Link href={href("/forecasts/")}>{established ? (sv ? "Prognos mot valresultat →" : "Forecast vs election result →") : (sv ? "Valnattens modellprognos →" : "Election-night model projection →")}</Link><Link href={href("/valnatt/#forecast-comparison")}>{sv ? "Jämför utfallet med förvalsprognosen →" : "Compare the result with the pre-election forecast →"}</Link></div>
    </section>
  </>;
}

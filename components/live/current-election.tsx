...
export function CurrentElection({ view = "table" }: { view?: "table" | "chart" | "indicators" }) {
  ...
  return <section className="interior-panel current-election" id="election-2026" data-classification={view === "indicators" ? "DERIVED" : "OFFICIAL"} data-result-revision={result?.sourceRevision}>
    <div className="panel-heading"><div><span className="mini-label">{view === "indicators" ? "DERIVED · pv-count-indicators-1.0.0" : (sv ? "OFFICIELLT · RIKSDAGEN" : "OFFICIAL · RIKSDAG")}</span><h2>{sv ? "2026 jämfört med 2022" : "2026 compared with 2022"}</h2>
    ...
    {!area ? <p>{sv ? "Inväntar verifierade röster." : "Waiting for verified votes."}</p> : <>
      {view === "chart" && <><p className="local-note">{sv ? "Fylld stapel: 2026 · ljus stapel: 2022. Andel giltiga röster." : "Solid bar: 2026 · light bar: 2022. Share of valid votes."}</p><div className="count-comparison-chart">{parties.map(p => {
      ...
      {view === "indicators" ? <>
        {indicators ? <div className="count-indicators"><article><span>{sv ? "Störst andelsökning" : "Largest share gain"}</span><strong>{name(indicators.gain.code)}</strong><b>{change(indicators.gain.change)} pp</b></article><article><span>{sv ? "Störst andelsminskning" : "Largest share loss"}</span><strong>{name(indicators.loss.code)}</strong><b>{change(indicators.loss.change)} pp</b></article><article><span>{sv ? "Nettorörlighet" : "Net volatility"}</span><strong>{f(indicators.volatility)} pp</strong><small>{sv ? "Halva summan av absoluta andelsförändringar, inklusive övriga." : "Half the sum of absolute share changes, including other parties."}</small></article></div> : <p>{sv ? "Jämförbart underlag för indikatorerna saknas ännu." : "Comparable data for these indicators is not yet available."}</p>}
        <p className="local-note">{sv ? "Beräknat från räknade röster. Räkningen 2026 jämförs med källans hela 2022-underlag. Nettorörlighet beskriver ändrade partistyrkor, inte hur enskilda väljare bytt parti. Utfallet kan ändras under räkningen." : "Calculated from counted votes. The 2026 count is compared with the source’s full 2022 baseline. Net volatility describes changes in party support, not individual voters switching parties. Results may change as counting continues."}</p>

...
export function LanguageSwitcher() {
  ...
  return <nav className="language-switcher" aria-label={locale === "sv" ? "Språk" : "Language"}>
    <a data-language-switch="true" href={`${base}${localizedHref(destination, "sv")}${route === "maps" ? query : ""}`} hrefLang="sv" lang="sv" aria-current={locale === "sv" ? "page" : undefined}>Svenska</a>
    <span aria-hidden="true">/</span>
    <a data-language-switch="true" href={`${base}${localizedHref(destination, "en")}${route === "maps" ? query : ""}`} hrefLang="en" lang="en" aria-current={locale === "en" ? "page" : undefined}>English</a>
  </nav>;
}
